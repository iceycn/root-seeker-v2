...
- `01-检查配置-插件-MCP-日志权限-外部平台.md`：检查配置/插件/MCP/日志权限/外部平台
- `02-cli_commands-commands-doctor.py.md`：单独定义 ``cli_commands/commands/doctor.py`` 的职责、输入输出和验收标准
- `03-status.py.md`：单独定义 ``status.py`` 的职责、输入输出和验收标准
