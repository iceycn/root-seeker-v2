...
def test_resolve_resume_step_index_maps_old_checkpoint_before_find_callers() -> None:
    # Old flow ended at notify; current flow inserted find-callers before notify.
    # Completed steps through code-read should resume at the new find-callers step,
    ...
    assert current_flow[mapped] == "find-callers"
    assert current_flow[10] == "find-callers"
    assert current_flow[11] == "notify"
...
def test_resolve_resume_step_index_skips_to_notify_when_find_callers_already_done() -> None:
    ...
    mapped = resolve_resume_step_index(
        current_steps=steps,
        current_next_step_index=2,
        flow_step_ids=["code-read", "find-callers", "notify"],
    )
    assert mapped == 2
    assert ["code-read", "find-callers", "notify"][mapped] == "notify"
...
def test_resolve_resume_step_index_inserts_find_callers_without_completed_statuses() -> None:
    ...
    current_flow = ["code-read", "find-callers", "notify"]
    mapped = resolve_resume_step_index(
        current_steps=old_steps,
